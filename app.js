...
app.get("/viewreport", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  try {
    ...
    // Get all courses created by the logged-in faculty
    const facultyCourses = await Course.findAll({
      where: {
        facultyid: loggedInUser.id,
      },
      include: [Chapter],  // Include chapters so we can get pages later
    });

    const courseReport = [];

    for (const course of facultyCourses) {
      // Count total number of enrollments for this course
      ...
      // Get all chapters and pages for this course
      const chapterIds = course.Chapters.map((chapter) => chapter.id);
      const totalPagesData = await Page.getPagesByChapterIds(chapterIds);
      const allPagesIdsForCourse = totalPagesData.map((page) => page.id);
      const totalPages = await Page.getPagesCountByChapterIds(chapterIds);

      // Get the number of students who have completed all pages in the course
      const completedStudents = await Progress.findAll({
        where: {
          pageid: allPagesIdsForCourse,
          iscompleted: true,
        },
        attributes: ['studentid'], // Get only student IDs
        group: ['studentid'], // Group by student ID
        having: sequelize.where(
          sequelize.fn('COUNT', sequelize.col('pageid')), totalPages // Count of completed pages must equal total pages
        ),
      });
      ...
      const inProgressStudents = totalEnrollments - completedStudentsCount;

      // Add the data for this course to the report
      courseReport.push({
