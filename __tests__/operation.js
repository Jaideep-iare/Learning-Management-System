...
const createUser = async (agent, role = "faculty") => {
  ...
  res = await agent.post("/users").send({
    ...
    role,
    email:
      role === "faculty" ? "faculty.user@test.com" : "student.user@test.com",
    password: "12345678",
    _csrf: csrfToken,
  });
  return res;
...
describe("Course operations by faculty suite", () => {
  ...
  test("Add chapter as faculty", async () => {
    ...
    const latestCourse =
      parsedgroupedResponse.facultyCourses[facultyCourseslen - 1];
  ...
  test("Add Page as faculty", async () => {
    ...
    groupedCourseResponse = await agent
      .get(`/addchapter/${latestCourse.id}`)
      .set("Accept", "application/json");
    ...
    const latestChapter =
      parsedgroupedResponse.getChaptersByCourse[courseChapterslen - 1];
  ...
  test("Delete Page as faculty", async () => {
    ...
    let chapterPages = allPages.filter(
      (page) => page.chapterid === latestChapter.id
    );
  ...
  test("Delete Chapter as faculty", async () => {
    ...
    const response = await agent
      .delete(`/deletechapter/${latestChapter.id}`)
      .send({
        _csrf: csrfToken,
      });
    expect(response.statusCode).toBe(200);
  ...
  test("Delete Course as faculty", async () => {
    ...
    const response = await agent
      .delete(`/deletecourse/${latestCourse.id}`)
      .send({
        _csrf: csrfToken,
      });
    expect(response.statusCode).toBe(200);
...
describe("Course operations by Student suite", () => {
  test("Preview the course", async () => {
    ...
    let latestAvailableCourse =
      parsedgroupedResponse.notEnrolledCourses[notEnrolledCourseslen - 1];
  ...
  test("Enroll the course", async () => {
    ...
    let latestAvailableCourse =
      parsedgroupedResponse.notEnrolledCourses[notEnrolledCourseslen - 3]; //to get the course which has chapter and page
